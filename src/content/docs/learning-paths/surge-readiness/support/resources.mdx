---
title: Support resources
pcx_content_type: overview
sidebar:
  order: 2
---

import { Plan } from "~/components"

<Plan type="ent-add-on" />

| Support type | Resource |
|------|------|
| Self-serve questions | https://support.cloudflare.com <br/> https://developers.cloudflare.com |
| Strategic guidance and best practices (proactive) | Reach out to your dedicated account team |
| Non-critical production issues (reactive) | - Email the 24/7 Enterprise Support team <br/> - [Support portal](https://dash.cloudflare.com/?to=/:account/support) (preferred method) <br/> - Dashboard chat |
| Critical issues such as attacks (reactive) | -Call the 24/7 Emergency Support line <br/> - +1 (650)353-5922 (US) <br/> - www.cloudflare.com/ecp/support (global lines)| 

:::note
For security reasons, Cloudflare Support only assists individuals whose email addresses are validated against the list of registered account contacts. Review and update all contacts accordingly in your Cloudflare Dashboard.
:::

## Additional resources 

- For help with an issue, refer to [guidance for submitting support tickets](/support/contacting-cloudflare-support/).
- Learn the basic countermeasures to [stop an ongoing DDoS attack](/ddos-protection/best-practices/respond-to-ddos-attacks/).
-  Let [Cloudflare's Security Operations Center-as-a-Service (SOC)](https://www.cloudflare.com/soc-as-a-service/) monitor your environment for security threats and potential operational disruptions, perform deep analysis to identify attack vectors, and help you implement countermeasures to mitigate future incidents. 
- Utilize the [Technical Account Management Service](https://www.cloudflare.com/technical-account-management-service/) which operates as an extension of your team, as the Cloudflare support expert who knows your tech stack, unique infrastructure, and Cloudflare portfolio requirements.
- Learn [what's new](www.cloudflare.com/whats-new/) and subscribe to product release email summaries. 
- Read the [Cloudflare blog](blog.cloudflare.com) for the latest announcements from Cloudflare.
- Refer to the [Cloudflare Community](community.cloudflare.com) to seek advice and share insights about using Cloudflare with other Cloudflare users.
