...
You can also configure selective Worker-first routing using an array of route patterns. This allows you to run the Worker first only for specific routes while letting other requests follow the default asset-first behavior:
...
```json
{
	...
	"assets": {
		...
		"run_worker_first": [
			...
			"/auth/**",         // All auth routes go to Worker first
...
Once the Worker has been invoked, you can defer to static-asset serving using an Assets binding:
