...
- `run_worker_first` <Type text="boolean | string[]" /> <MetaInfo text="optional, defaults to false" />
  ...
  - Controls whether static assets are fetched directly, or a Worker script is invoked. Can be a boolean (`true`/`false`) or an array of route patterns with support for glob patterns (`**`) and exception patterns (`!` prefix). Learn more about fetching assets when using [`run_worker_first`](/workers/static-assets/routing/worker-script/#run-your-worker-script-first).
